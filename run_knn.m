...
[sorted_dist, nearest] = sort(dist,2); %sort each row in ascending order

nearest = nearest(:,1:k); %pick the columns corresponding to the smallest 
%k distances between valid data and train data.
valid_labels = train_labels(nearest);

% note this only works for binary labels

%mean(A,2) means that it return the mean of each row, and compose them into a column vector
